...
| `alumniSubmissions` | Temporary alumni profile collection form submissions. | Public Create / Admin Full |
...
### Alumni submissions
**Purpose**: Temporary collection of alumni details before they are reviewed and published to the public alumni section.
**Document ID**: Auto-generated
> **Note**: Public create is allowed for validated submissions; read, update, and delete remain admin-only.
```json
{
  "name": "String",                  // max 100 chars
  "email": "String",                 // max 100 chars
  ...
  "linkedin": "String",              // URL (Optional)
  "otherSocial": "String",           // URL (Optional)
...
service cloud.firestore {
  match /databases/{database}/documents {
    ...
    match /alumniSubmissions/{submissionId} {
      allow create: if isValidString(request.resource.data.name, 100)
                    && isValidString(request.resource.data.email, 100)
                    && isValidString(request.resource.data.graduationYear, 4)
                    && isValidString(request.resource.data.currentCompany, 120)
                    && isValidString(request.resource.data.currentRole, 120)
                    && isOptionalString(request.resource.data.linkedin, 200)
                    && isOptionalString(request.resource.data.otherSocial, 200)
