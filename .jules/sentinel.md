...
## 2025-03-05 - XSS via innerHTML in React onError Handlers
**Vulnerability:** Found `e.currentTarget.parentElement.innerHTML = ...` assignments inside `onError` attributes of `<img>` tags in React components (`src/pages/Events.jsx`, `src/pages/Team.jsx`). In one instance, user data (`member.name`) was concatenated directly into the HTML string, exposing a Cross-Site Scripting (XSS) vulnerability if the user data contained malicious HTML.
**Learning:** React developers sometimes bypass React's virtual DOM and use raw DOM manipulation (like `innerHTML`) for quick fixes (e.g., fallback images). This defeats React's built-in XSS protection and introduces classic injection vulnerabilities, especially when mapping over data and inserting object properties.
**Prevention:** Never use `innerHTML` (or `dangerouslySetInnerHTML`) to dynamically inject user data. Instead of manipulating the DOM directly on error, use React state (`useState`) to track the error and conditionally render a fallback component, or use CSS display toggling (`e.currentTarget.style.display = 'none'; e.currentTarget.nextElementSibling.style.display = 'flex';`) with a pre-rendered, hidden sibling element.
