...
## 2024-05-24 - [MEDIUM] Sanitize input display in AdminMessages
**Vulnerability:** The AdminMessages view was directly rendering `msg.name`, `msg.email`, and `msg.message` and passing `msg.email` into a `mailto:` `href`. This allowed malicious payload interpolation (e.g. data URI injection or rendering of internal objects if type validation fails) because React does not sanitize object-to-string coercion or URL schemes internally in all contexts.
**Learning:** Even internal admin dashboards rendering non-sanitized DB contents can be an attack vector (Stored XSS or Data Exfiltration) if data gets corrupted or bypasses frontend validation.
**Prevention:** Always wrap DB content interpolations with security utilities (like `safeRender` and `safeHref` in `src/utils/security.js`) when displaying on UI, particularly for user-input driven collections.
