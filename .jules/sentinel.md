...
## 2024-06-18 - [XSS] Unsanitized Dynamic URLs in React Components
**Vulnerability:** Potential Cross-Site Scripting (XSS) via JavaScript URI injection (e.g., `javascript:alert(1)`) in dynamic `<a>` tag `href` attributes within React components (`Team.jsx`, `HomeTeam.jsx`, `Projects.jsx`).
**Learning:** React does not automatically sanitize `href` attributes against `javascript:` URIs. If user-controlled data (like social media links or project URLs) is directly interpolated into `href`, it can lead to XSS execution when clicked.
**Prevention:** Always validate and sanitize dynamic URLs used in `href` attributes using a utility function like `safeHref` to ensure they only contain safe protocols (e.g., `http:`, `https:`, `mailto:`, `tel:`).
