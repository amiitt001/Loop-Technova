...
## $(date +%Y-%m-%d) - [Schema Pollution] Restrict allowed keys in public Firestore writes
**Vulnerability:** The public `messages` collection allowed any authenticated or unauthenticated user to write documents. While some fields were validated for type and length (`name`, `email`, `message`, `read`), the rules did not restrict the allowed keys. An attacker could inject arbitrary data into the collection (e.g., `admin: true`, `role: "superuser"`, or large amounts of garbage data), potentially exploiting type confusion, bypassing downstream logic, or causing storage exhaustion.
**Learning:** Firestore's `allow create` rule must not only validate expected fields but also explicitly restrict the allowed keys using `request.resource.data.keys().hasOnly([...])` to prevent schema pollution, especially on public or low-privilege endpoints.
**Prevention:** Always use `request.resource.data.keys().hasOnly()` to strictly define the allowed schema for any publicly writable collection in Firestore security rules.
