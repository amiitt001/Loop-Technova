...
## 2024-06-05 - [Authorization Bypass via Client-Side DB Write]
**Vulnerability:** The `RegistrationModal.jsx` component contained logic for a direct `addDoc` to the `registrations` Firestore collection from the client side. This completely bypasses the backend API (`/api/register`), which enforces crucial security checks, such as duplicate validation, length restrictions, and input sanitization to prevent downstream attacks (like CSV/Formula Injection in Google Sheets).
**Learning:** Client-side application logic should never perform direct database writes for sensitive collections. Bypassing the backend API neutralizes all server-side validation and sanitization, leaving the database vulnerable to malicious input and data corruption.
**Prevention:** Always ensure frontend components communicate through secure backend API endpoints. Remove or refactor any legacy direct database writes, and configure Firestore Security Rules to deny public writes to protected collections.
