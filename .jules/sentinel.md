...
## 2025-02-18 - [HIGH] Sanitize User Input in Admin Messages View
**Vulnerability:** The Admin Messages panel directly rendered raw user inputs from Firestore (`msg.name`, `msg.email`, `msg.message`) without explicitly wrapping them. While React typically escapes strings, failing to use `safeRender` allows objects to be rendered directly into the DOM (causing application crashes) or permits potential XSS if the data structures are manipulated directly in Firestore. Furthermore, `mailto:` links were concatenated using raw input (`href={"mailto:" + msg.email}`), creating a javascript URI XSS injection vulnerability.
**Learning:** Even internal, "admin-only" views must implement defense in depth. An attacker submitting a malicious code snippet via a public contact form could crash the admin panel or trigger XSS when the admin tries to click the user's email link. The `safeHref` method must always be used for generated links, especially those using user data.
**Prevention:** Always use the `safeRender` and `safeHref` utility functions imported from `src/utils/security.js` whenever displaying user-generated data or populating attributes, regardless of what backend sanitization is theoretically performed.
