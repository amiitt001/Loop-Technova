...
## 2026-04-09 - [XSS] Removed dynamic innerHTML usage in React components
...
## 2026-04-09 - [CSV Injection] Unsanitized client-side CSV exports
...
## 2026-04-09 - Arbitrary Timestamp Injection in Firestore
**Vulnerability:** The Firestore security rules for the public `messages` collection allowed the `createdAt` field to be set to any arbitrary client-provided timestamp.
**Learning:** Checking for the presence of a timestamp field or its type does not prevent an attacker from supplying a false time. This can lead to sorting manipulation, bypassing time-based protections, or data inconsistencies.
**Prevention:** Always enforce server-side timestamps for critical temporal fields in Firestore rules by explicitly checking `request.resource.data.createdAt == request.time`.
