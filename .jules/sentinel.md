...
## 2025-10-28 - [XSS] Missing Output Sanitization in Admin Messages
**Vulnerability:** User-generated messages from the Contact form (`msg.name`, `msg.email`, `msg.message`) were directly rendered in the Admin Panel without wrapping them in the existing `safeRender` and `safeHref` utility functions. Although React escapes text to prevent direct HTML/script injection, it does not prevent JavaScript URI injection in `href` links (e.g. `mailto:javascript:alert(1)`) nor does it prevent the application from crashing if a raw object or JSON blob is rendered.
**Learning:** Admin interfaces are prime targets for Stored XSS or Denial of Service (via React rendering errors) because they frequently display untrusted data from public forms.
**Prevention:** Always use `safeRender` when rendering any user-generated string to ensure it's a primitive and not an unsafe pattern. Always use `safeHref` to validate that dynamically generated links (including `mailto:` links) do not contain malicious URI schemes.
